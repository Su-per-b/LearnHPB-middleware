...
static void XMLCALL startElement(void *context, const char *elm, const char **attr) {
    ...
    switch(getAstNodeType(el)){
		...
		default: assert(0);   break; // Error message if there is no matching element,
    }
...
////////////////////////////////////////////////////////////////////////////////////
void printidf(ModelDescription* md)
{
	FILE *fp;
	char fmuname[100], type[12];
	int i, j, varname, vardes;
	void **list;
	ScalarVariable* se;
	Element* ee;
  ...
  // Define ExternalInterface:FunctionalMockupUnit
	...
	for(i=0; i<md->n; i+=2)
		if(!strcmp(md->attributes[i], "modelIdentifier"))
			strcpy(fmuname, md->attributes[i+1]);

	fprintf(fp, "  %s.fmu,\t\t!- FMU File Name\n", fmuname);
  fprintf(fp, "   ,\t\t!- FMU Model Name\n");
  fprintf(fp, "   ,\t\t!- FMU Timeout Value\n");
  ...
  fprintf(fp, "   ;\t\t!- FMU LoggingOn Value\n");
	...
	if (list)
      ...
      // Define part of ExternalInterface:FunctionalMockupUnit:To      
            ...
            fprintf(fp, "   ,\t\t!- EnergyPlus Key Variable Value\n");          
						break;
					case enu_output:
        ...
        fprintf(fp, "   ,\t\t!- Energyplus Variable Name\n");
				...
				{
          case enu_input:
            fprintf(fp, "   %s;\t\t!- FMU Model Variable Name", e->attributes[varname]);
            break;
					case enu_output:
            fprintf(fp, "   %s,\t\t!- FMU Model Variable Name", e->attributes[varname]);				   
            break;
        ...
        // Get the type of FMU variable 
				...
				fprintf(fp, ", %s", e->attributes[vardes]);
