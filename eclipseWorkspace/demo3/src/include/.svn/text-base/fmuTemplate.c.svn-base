...
const char* fmiGetTypesPlatform() {			// Zuo: Definition in V1.0
	return fmiTypesPlatform;				// Zuo: Definition in V1.0
}
...
fmiComponent fmiInstantiateSlave(fmiString instanceName, fmiString GUID, 
		...
		printf("Start to instantiate model\n");		
    ModelInstance* comp;
		printf("Start to instantiate model\n");
    if (!functions.logger) 
        return NULL;
    if (!functions.allocateMemory || !functions.freeMemory){ 
        functions.logger(NULL, instanceName, fmiError, "error", 
                //"fmiInstantiateModel: Missing callback function.");	// Zuo: model-ex
				"fmiInstantiateSlave: Missing callback function.");		// Zuo: co-sim
        return NULL;
    }
    if (!instanceName || strlen(instanceName)==0) { 
        functions.logger(NULL, instanceName, fmiError, "error", 
                //"fmiInstantiateModel: Missing instance name.");		// Zuo: model-ex
				"fmiInstantiateSlave: Missing instance name.");			// Zuo: co-sim
        return NULL;
    }
    if (strcmp(GUID, MODEL_GUID)) {
        functions.logger(NULL, instanceName, fmiError, "error", 
                //"fmiInstantiateModel: Wrong GUID %s. Expected %s.", GUID, MODEL_GUID);	// Zuo: model-ex
				"fmiInstantiateSlave: Wrong GUID %s. Expected %s.", GUID, MODEL_GUID);		// Zuo: co-sim
        return NULL;
    }
    ...
    if (!comp || !comp->r || !comp->i || !comp->b || !comp->s || !comp->isPositive) {
        functions.logger(NULL, instanceName, fmiError, "error", 
                //"fmiInstantiateModel: Out of memory.");	// Zuo: model-ex
				"fmiInstantiateSlave: Out of memory.");		// Zuo: co-sim
        return NULL;
    }
    if (comp->loggingOn) comp->functions.logger(NULL, instanceName, fmiOK, "log", 
            //"fmiInstantiateModel: GUID=%s", GUID);	// Zuo: model-ex
			"fmiInstantiateSlave: GUID=%s", GUID);		// Zuo: co-sim
...
//void fmiFreeModelInstance(fmiComponent c) {		// Zuo: definition in model-ex V1.0
void fmiFreeSlaveInstance(fmiComponent c) {			// Zuo: definition in co-sim V1.0
    ModelInstance* comp = (ModelInstance *)c;
    if (!comp) return;
    if (comp->loggingOn) comp->functions.logger(c, comp->instanceName, fmiOK, "log", 
            //"fmiFreeModelInstance");				// Zuo: model-ex
			"fmiFreeSlaveInstance");				// Zuo: co-sim
...
// Zuo: the following function is modifed based on ITI's fmiDoStep in ITI_fmiCSFunctions.c
...
//    fmiEventInfo* eventInfo) {														// Zuo: definition in model-ex V1.0
fmiStatus fmiInitializeSlave(fmiComponent c, fmiReal tStart,
    ...
    ModelInstance* comp = (ModelInstance *)c;
    if (invalidState(comp, "fmiInitializeSlave", modelInstantiated))
         return fmiError;
    //if (nullPointer(comp, "fmiInitialize", "eventInfo", eventInfo))								// Zuo: definition in model-ex V1.0
    //     return fmiError;																			// Zuo: Fixme need new funciton
    //if (comp->loggingOn) comp->functions.logger(c, comp->instanceName, fmiOK, "log", 
    //    "fmiInitialize: toleranceControlled=%d relativeTolerance=%g",		
    //    toleranceControlled, relativeTolerance);													// Zuo: Fixme need new funciton
    //eventInfo->iterationConverged  = fmiTrue;
    //eventInfo->stateValueReferencesChanged = fmiFalse;
    //eventInfo->stateValuesChanged  = fmiFalse;
    //eventInfo->terminateSimulation = fmiFalse;
    //eventInfo->upcomingTimeEvent   = fmiFalse;
    //initialize(comp, eventInfo);		// Zuo Fixme: to be implemented by the includer of this file
    comp->state = modelInitialized;
    return fmiOK;
}

// Zuo: definition in model-ex V1.0 but not in co-sim V1.0 
