...
First implementation.
        ...
        partial model PartialFlowMachine
          ...
          parameter Modelica.Media.Interfaces.PartialMedium.SpecificEnthalpy
            ...
            annotation (Dialog(tab="Initialization"));
          parameter Modelica.Media.Interfaces.PartialMedium.MassFraction
            X_start[                                                             Medium.nX]=
             Medium.X_default "Start value of mass fractions m_i/m"
            annotation (Dialog(tab="Initialization"));
          parameter Modelica.Media.Interfaces.PartialMedium.ExtraProperty
            C_start[
...
First implementation.
      ...
      package BaseClasses "Library with base classes for psychrometric classes"
        extends Modelica.Fluid.Icons.BaseClassLibrary;
...
Buildings.Utilities.IO.BCVTB.From_degC from_degC1
  ...
  Modelica.Blocks.Interfaces.RealInput TDryBul
    ...
    annotation (Placement(transformation(extent={{-136,76},{-98,114}})));

  Modelica.Blocks.Interfaces.RealInput TRooMea
    "Mean temperature of room in Celsius"
    annotation (Placement(transformation(extent={{-134,12},{-100,46}})));

  Modelica.Blocks.Interfaces.RealInput outRelHum "Outdoor relative humidity"
    annotation (Placement(transformation(extent={{-136,42},{-100,78}})));

  Modelica.Blocks.Interfaces.RealInput rooRelHum "Room relative humidity"
    annotation (Placement(transformation(extent={{-136,-22},{-98,16}})));

  Modelica.Blocks.Interfaces.RealOutput QSensible
    "Sensible enthalpy flow rate, positive if flow into the component"
    annotation (Placement(transformation(extent={{460,48},{490,78}})));

  Modelica.Blocks.Interfaces.RealOutput QLatent
    "Latent enthalpy flow rate, positive if flow into the component"
    annotation (Placement(transformation(extent={{460,-16},{492,16}})));
...
connect(masFra.X, sou.X_in)    annotation (Line(
  ...
  connect(TRooSetNig.y, switch1.u3) annotation (Line(
      points={{61,128},{70,128},{70,152},{82,152}},
      color={0,0,127},
      smooth=Smooth.None));
...
connect(from_degC.Kelvin, bouBCVTB.T_in) annotation (Line(
    points={{41,29.8},{80,29.8},{80,12},{202,12}},
    color={0,0,127},
    smooth=Smooth.None));
connect(from_degC1.Kelvin, sou.T_in) annotation (Line(
    points={{21,95.8},{80,95.8},{80,74},{94,74}},
    color={0,0,127},
    smooth=Smooth.None));
connect(from_degC1.Kelvin, masFra.T) annotation (Line(
    points={{21,95.8},{40,95.8},{40,66},{48,66}},
    color={0,0,127},
    smooth=Smooth.None));
  connect(from_degC1.Celsius, TDryBul) annotation (Line(
      points={{-2,95.6},{-52,95.6},{-52,95},{-117,95}},
      color={0,0,127},
      smooth=Smooth.None));
  connect(TRooMea, from_degC.Celsius) annotation (Line(
      points={{-117,29},{-45,29},{-45,29.6},{18,29.6}},
      color={0,0,127},
      smooth=Smooth.None));
  connect(perToRel1.u, outRelHum) annotation (Line(
      points={{-2,60},{-118,60}},
      color={0,0,127},
      smooth=Smooth.None));
  connect(perToRel.u, rooRelHum) annotation (Line(
      points={{18,0},{-44,0},{-44,-3},{-117,-3}},
      color={0,0,127},
      smooth=Smooth.None));
  connect(bouBCVTB.HSen_flow, QSensible) annotation (Line(
      points={{225,15},{420,14},{420,63},{475,63}},
      color={0,0,127},
      smooth=Smooth.None));
  connect(bouBCVTB.HLat_flow, QLatent) annotation (Line(
      points={{225,12},{349.5,12},{349.5,0},{476,0}},
      color={0,0,127},
      smooth=Smooth.None));
  annotation (Diagram(coordinateSystem(preserveAspectRatio=true, extent={{-100,
