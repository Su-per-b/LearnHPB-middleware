...
// Attributes
// The entries starting at elm_Implementation were added by M. Wetter to support
// the FMI for co-simulation.
typedef enum {
    ...
    elm_Implementation, elm_CoSimulation_StandAlone, elm_Capabilities,
    elm_CoSimulation_Tool, elm_CoSimulationModel, elm_File
} Elm;

// Attributes
// The entries starting at att_canHandleVariableCommunicationStepSize were added by M. Wetter to support
// the FMI for co-simulation
typedef enum { 
  ...
  att_maxOutputDerivativeOrder, att_canRunAsynchronuously, att_canSignalEvents,
  att_canBeInstantiatedOnlyOncePerProcess, att_canNotUseMemoryManagementFunctions,
  att_entryPoint, att_manualStart, att_type, att_file
} Att;
...
typedef struct {
    ...
    ListElement** implementation;     // Zuo: NULL or null-terminated list of CoSimulation_StandAlone 
} ModelDescription;
...
typedef enum { 
    ...
    astType,
    astScalarVariable,
    astImplementation, // added M. Wetter
    astModelDescription,
} AstNodeType;
