...
//	const char* valueChar = getString(scalarVariable->typeSpec, att_e);
	...
	//const char* value = getString(scalarVariable->typeSpec, att_);
	...
	if (!valueChar) {
		*valueStatus = valueMissing;
		...
		if (!strcmp(valueChar, "true")) {
			*valueStatus = valueDefined;
			valueBool = 1;
		} else if (!strcmp(valueChar, "false")) {
			*valueStatus = valueDefined;
			valueBool = 0;
		} else {
...
ModelDescription* validate(ModelDescription* md) {
    ...
    for (i=0; md->modelVariables[i]; i++){
        ScalarVariable* sv = (ScalarVariable*)md->modelVariables[i];
        const char* declaredTypeChar = getString(sv->typeSpec, att_declaredType);
        Type* declaredTypeStruct = getDeclaredType(md, declaredTypeChar);
        if (declaredTypeChar && declaredTypeStruct==NULL) {
            printf("Warning: Declared type %s of variable %s not found in modelDescription.xml\n", declaredTypeChar, getName(sv));
            error++;
        }
    }
    if (error) {
